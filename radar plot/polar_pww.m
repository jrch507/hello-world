function hpol = polar_pww(varargin)
...
%     error(nargchk(1, 3, nargs, 'struct'));
    ...
    if nargs < 1 || (nargs>3 && mod(nargs,2)==1)
    ...
    elseif nargs>3 && mod(nargs,2)==0
        theta = args{1};
        rho = args{2};
        line_setting = {args{3:end}};
        line_style = 'manual';
    else % nargs == 3
    ...
    elseif strcmp(line_style,'manual')
        evalstr = strcat('q = plot(xx,yy');
        for k = 1:(nargs-2)/2
            if ischar(line_setting{k*2})
                evalstr = strcat(evalstr,',''',line_setting{(k-1)*2+1},''',''',line_setting{k*2},'''');
            else
            evalstr = strcat(evalstr,',''',line_setting{(k-1)*2+1},''',','line_setting{',num2str(k*2),'}');
            end
        end
